...
void register_debug_routes(c_http_router& router) {
    ...
    // POST /api/debug/init - Initialize (load) a debuggee
    router.post("/api/debug/init", [](const s_http_request& req) -> s_http_response {
        auto body = nlohmann::json::parse(req.body, nullptr, false);
        if (body.is_discarded()) {
            return s_http_response::bad_request("Invalid JSON body");
        }

        auto path = body.value("path", std::string());
        auto arguments = body.value("arguments", std::string());
        auto current_dir = body.value("current_dir", std::string());

        // Build init command: init <path>, [arguments], [current_dir]
    ...
    // POST /api/debug/attach - Attach to a process by PID
    router.post("/api/debug/attach", [](const s_http_request& req) -> s_http_response {
        auto body = nlohmann::json::parse(req.body, nullptr, false);
        if (body.is_discarded() || !body.contains("pid")) {
            return s_http_response::bad_request("Missing 'pid' field");
        }

        auto pid = body["pid"].get<uint32_t>();
        auto& bridge = get_bridge();

        auto cmd = "attach " + std::to_string(pid);
        auto success = bridge.exec_command(cmd);
    ...
    // POST /api/debug/detach - Detach from process
    router.post("/api/debug/detach", [](const s_http_request&) -> s_http_response {
        auto& bridge = get_bridge();
        auto success = bridge.exec_command("detach");

        return s_http_response::ok({
            {"success", success},
            {"message", "Detach attempted"}
        });
    });

    // POST /api/debug/undo - Undo last instruction step (InstrUndo)
    router.post("/api/debug/undo", [](const s_http_request&) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_paused()) {
            return s_http_response::conflict("Debugger must be paused");
        }
        auto success = bridge.exec_command("InstrUndo");
        return s_http_response::ok({{"success", success}});
    });

    // POST /api/debug/run_to_user_code - Run until user code (RunToUserCode/rtu)
    router.post("/api/debug/run_to_user_code", [](const s_http_request&) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_paused()) {
            return s_http_response::conflict("Debugger must be paused");
        }
        auto success = bridge.exec_command("RunToUserCode");
        return s_http_response::ok({{"success", success}});
    });
}
