...
void register_command_routes(c_http_router& router) {
    ...
    // POST /api/command/execute_batch - Execute multiple commands, return aggregate results
    router.post("/api/command/execute_batch", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
    ...
    // GET /api/command/last_result - Get last command result
    router.get("/api/command/last_result", [](const s_http_request&) -> s_http_response {
        // x64dbg stores the last result in $result
        auto& bridge = get_bridge();
