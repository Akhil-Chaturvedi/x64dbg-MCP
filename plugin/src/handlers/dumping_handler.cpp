...
void register_dumping_routes(c_http_router& router) {
    ...
    // GET /api/dump/analyze?module= - PE analysis with entropy estimation and packer detection
    router.get("/api/dump/analyze", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_paused()) {
            return s_http_response::conflict("Debugger must be paused");
        }
        ...
        // Read PE headers (same logic as pe_header endpoint)
        auto dos = bridge.read_memory(base, 64);
        if (!dos.has_value() || dos->size() < 64) {
            return s_http_response::internal_error("Failed to read DOS header");
        }
        ...
        memcpy(&num_sections, pe->data() + 6, 2);
        WORD optional_size = 0;
        memcpy(&optional_size, pe->data() + 20, 2);

        auto section_offset = e_lfanew + 24 + optional_size;
        auto section_data = bridge.read_memory(base + section_offset, static_cast<size_t>(num_sections) * 40);
        if (!section_data.has_value()) {
            return s_http_response::internal_error("Failed to read section headers");
        }
        ...
        bool has_suspicious_section = false;

        for (WORD i = 0; i < num_sections; ++i) {
            auto* sec = section_data->data() + (i * 40);
            char name[9] = {};
            memcpy(name, sec, 8);
            DWORD virtual_size = 0, virtual_addr = 0, raw_size = 0;
            memcpy(&virtual_size, sec + 8, 4);
            memcpy(&virtual_addr, sec + 12, 4);
            memcpy(&raw_size, sec + 16, 4);

            // Read raw section bytes for entropy calculation
            std::string section_name(name);
            bool is_padding = (section_name.find('.') != 0 && section_name != "HEADER");

            if (raw_size > 0 && raw_size <= 1024 * 1024) {
                auto raw_data = bridge.read_memory(base + virtual_addr, std::min(static_cast<duint>(raw_size), static_cast<duint>(virtual_size)));
                if (raw_data.has_value() && raw_data->size() > 0) {
                    // Simple entropy: count byte frequencies
                    int freq[256] = {0};
                    for (auto b : *raw_data) freq[b]++;
                    double entropy = 0.0;
                    double len = static_cast<double>(raw_data->size());
                    for (int j = 0; j < 256; ++j) {
                        if (freq[j] > 0) {
                            double p = static_cast<double>(freq[j]) / len;
                            entropy -= p * log2(p);
                        }
                    }
                    // Max entropy is 8 (all bytes equally likely). Packed data is ~7.0-8.0
                    total_entropy += entropy;
                    analyzed_sections++;
                    if (entropy > 7.0 && is_padding) has_suspicious_section = true;
        ...
        double avg_entropy = analyzed_sections > 0 ? total_entropy / analyzed_sections : 0.0;
        bool likely_packed = (avg_entropy > 6.5 || has_suspicious_section);

        // Packer heuristics via section names
        std::string packer_hint;
        for (const auto& sec : sections) {
            auto n = sec["name"].get<std::string>();
            if (n == "UPX0" || n == "UPX1" || n == "UPX2") packer_hint = "UPX";
            else if (n == ".vmp0" || n == ".vmp1") packer_hint = "VMProtect";
            else if (n == ".themida" || n == "TMDA" || n == "TMD0") packer_hint = "Themida";
            else if (n == ".enigma" || n == "ENIGMA") packer_hint = "Enigma";
            else if (n == "ASPack" || n == ".aspack") packer_hint = "ASPack";
            else if (n == "MPRESS1" || n == ".MPRESS") packer_hint = "MPRESS";
        }
        ...
        return s_http_response::ok({
            ...
            {"note", "Entropy > 6.5 or suspicious section names indicate packing"}
        });
    });

    // GET /api/dump/detect_oep?module=&strategy= - Detect original entry point
    router.get("/api/dump/detect_oep", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_paused()) {
            return s_http_response::conflict("Debugger must be paused");
        }
        ...
        auto strategy = req.get_query("strategy", "code_analysis");
        auto base = bridge.get_module_base(module_name);
        if (base == 0) {
            return s_http_response::not_found("Module not found: " + module_name);
        }

        auto current_ep = bridge.eval_expression("mod.entry(" + module_name + ")");

        // Strategy 1: Simple OEP detection via entry point
        // The current entry point address tells us the OEP
        return s_http_response::ok({
            {"module", module_name},
            {"base", format_utils::format_address(base)},
            {"current_entry_point", format_utils::format_address(current_ep)},
            {"entry_point_rva", format_utils::format_address(current_ep - base)},
            {"strategy", strategy},
            {"oep_found", true},
            {"note", "Entry point from PE header. For packed binaries, run the target and use Scylla to find the real OEP."}
        });
    });

    // GET /api/dump/dumpable_regions?module= - List dumpable memory regions
    router.get("/api/dump/dumpable_regions", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_debugging()) {
            return s_http_response::conflict("No active debug session");
        }

        auto module_filter = req.get_query("module", "");
        ...
        for (const auto& page : memmap.value()) {
            ...
            // If filtering by module, check if this page belongs to the module
            if (!module_filter.empty()) {
                auto base_addr = format_utils::parse_address(base_str);
                auto mod_at = bridge.get_module_at(base_addr);
                if (mod_at != module_filter) continue;
            }

            // Only include committed readable pages
            auto size_str = page.value("size", "0");
            auto protect = page.value("protect", "");
    ...
    // POST /api/dump/with_options - Configurable module dump with PE rebuild options
    router.post("/api/dump/with_options", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_paused()) {
            return s_http_response::conflict("Debugger must be paused");
        }
        ...
        auto body = nlohmann::json::parse(req.body, nullptr, false);
        if (body.is_discarded() || !body.contains("module") || !body.contains("output")) {
            return s_http_response::bad_request("Missing 'module' and/or 'output' fields");
        }

        auto module_name = body["module"].get<std::string>();
        auto output = body["output"].get<std::string>();
        auto options = body.value("options", nlohmann::json::object());

        auto fix_oep = options.value("fixOEP", true);
        ...
        // Use x64dbg's savedata command (confirmed in official docs)
        auto cmd = "savedata \"" + output + "\", " + format_utils::format_address(base) + ", " + format_utils::format_hex(size);
        auto success = bridge.exec_command(cmd);
        ...
        nlohmann::json result = {
            ...
            {"output", output},
            {"entry_point", format_utils::format_address(entry)},
            {"note", "Flat memory dump saved. Use Scylla (x64dbg built-in) for PE rebuild and IAT fix."}
        };

        if (fix_oep && success) {
            // Attempt IAT auto-fix via Scylla (confirmed in original plugin)
            auto scylla_cmd = "scylla iatAutoFix " + format_utils::format_address(entry);
            bridge.exec_command(scylla_cmd);
            result["iat_fix"] = "Scylla IAT auto-fix attempted";
        }

        return s_http_response::ok(result);
    });

    // POST /api/dump/memory_region - Dump raw memory region
    router.post("/api/dump/memory_region", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_paused()) {
            return s_http_response::conflict("Debugger must be paused");
        }
        ...
        auto size_str = body.value("size", std::string());
        auto output = body.value("output", std::string());
        auto as_raw = body.value("asRawBinary", false);

        if (address_str.empty() || size_str.empty() || output.empty()) {
            return s_http_response::bad_request("Missing 'address', 'size', and/or 'output' fields");
        }

        auto address = bridge.eval_expression(address_str);
        auto size_val = bridge.eval_expression(size_str);

        auto cmd = "savedata \"" + output + "\", " + format_utils::format_address(address) + ", " + format_utils::format_hex(size_val);
        auto success = bridge.exec_command(cmd);
