...
void register_analysis_routes(c_http_router& router) {
    ...
    // GET /api/analysis/functions?module= - List all recognized functions
    router.get("/api/analysis/functions", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_debugging()) {
            return s_http_response::conflict("No active debug session");
        }
        ...
        BridgeList<Script::Function::FunctionInfo> funcList;
        if (!Script::Function::GetList(&funcList)) {
            return s_http_response::ok({{"functions", nlohmann::json::array()}, {"count", 0}});
        }

        auto result = nlohmann::json::array();
        for (int i = 0; i < funcList.Count(); ++i) {
            const auto& f = funcList[i];

            // Apply optional module filter
            if (!module_filter.empty()) {
                if (module_filter != f.mod) continue;
            }

            char label[MAX_LABEL_SIZE] = {};
            DbgGetLabelAt(f.rvaStart, SEG_DEFAULT, label);

            result.push_back({
                {"start",   format_utils::format_address(f.rvaStart)},
                {"end",     format_utils::format_address(f.rvaEnd)},
                {"size",    f.rvaEnd - f.rvaStart},
    ...
    // GET /api/analysis/function_detail?address= - Get function boundaries at address
    router.get("/api/analysis/function_detail", [](const s_http_request& req) -> s_http_response {
        auto& bridge = get_bridge();
        if (!bridge.require_paused()) {
            return s_http_response::conflict("Debugger must be paused");
        }
        ...
        if (!Script::Function::GetInfo(address, &info)) {
            return s_http_response::not_found("No function found at address");
        }

        char label[MAX_LABEL_SIZE] = {};
        DbgGetLabelAt(info.rvaStart, SEG_DEFAULT, label);

        return s_http_response::ok({
            {"start",   format_utils::format_address(info.rvaStart)},
            {"end",     format_utils::format_address(info.rvaEnd)},
            {"size",    info.rvaEnd - info.rvaStart},
            {"label",   label[0] ? std::string(label) : ""},
            {"module",  info.mod},
            {"manual",  info.manual}
        });
