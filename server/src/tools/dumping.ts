...
export function registerDumpingTools(server: McpServer) {
  server.tool('x64dbg_dumping', 'Dump modules, fix IAT, or get PE header/sections/imports/exports/relocations', {
    action: z.discriminatedUnion("action", [
      ...
      z.object({ action: z.literal("export_patch_file"), filename: z.string() }),
        z.object({ action: z.literal("minidump"), file: z.string().optional() })
      ])
  }, async ({ action }) => {
    let data: any;
    switch (action.action) {
      ...
      case 'export_patch_file': data = await httpClient.post('/api/patches/export_file', { filename: action.filename }); break;
        case 'minidump': data = await httpClient.post('/api/dump/minidump', { file: action.file }); break;
    }
